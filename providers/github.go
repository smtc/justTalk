...
func (p *GitHubProvider) GetProfile(ss *SessionState) (profile []byte, err error) {
	endpoint := "https://api.github.com/user"
	req, _ := http.NewRequest("GET", endpoint, nil)
	req.Header.Set("Accept", "application/vnd.github.v3+json")
	req.Header.Add("Authorization", "token "+ss.AccessToken)
	...
	if resp.StatusCode != 200 {
		err = fmt.Errorf("got %d from %q %s", resp.StatusCode, endpoint, string(profile))
		return
	}
